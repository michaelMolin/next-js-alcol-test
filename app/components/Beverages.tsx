        ...
        <div>
            <main className="flex flex-col justify-between items-center h-screen bg-[#F7FAFF]">
                <div className="mt-9 w-full h-85/100 flex flex-col">
                    ...
                    <div className="px-2 pt-5 flex items-center overflow-x-auto mb-4 shrink-0">
                        {categories.map((category) => (
                            <div
                                key={category.id}
                                onClick={() => scrollToCategory(category.id)}
                                className={`cursor-pointer ${selectedCategoryId == category.id ? categorySelectedClass : categoryBaseClasse}`}
                            >
