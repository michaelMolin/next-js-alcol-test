    ...
    return (
        <div className="w-1/2 flex justify-between items-center">
                <button type="button" id="decrement-button" data-input-counter-decrement="quantity-input"
                        className="bg-[#F7FAFF] mr-1 text-display-1 font-bold w-1/4 font-title flex justify-center"
                ...
                <input type="text" id="quantity-input" data-input-counter aria-describedby="helper-text-explanation"
                       className="text-center bg-[#F7FAFF] mr-1 text-display-1 font-bold w-1/2 font-title"
                       value={currentDataBeverage.quantity}
                       onChange={handleInputChange}
                   required/>
                <button type="button" id="increment-button" data-input-counter-increment="quantity-input"
                        className="bg-[#F7FAFF] mr-1 text-display-1 font-bold w-1/4 text-center font-title flex justify-center"
                    onClick={() => { props.onQuantityChange(props.productId, currentDataBeverage.quantity + 1) }}
                >
                    <svg className="w-4 h-4 text-heading" aria-hidden="true" xmlns="http://www.w3.org/2000/svg"
                         width="24" height="24" fill="none" viewBox="0 0 24 24">
                        <path stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2"
                              d="M5 12h14m-7 7V5"/>
                    </svg>
